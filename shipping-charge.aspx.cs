...
public partial class auth_shipping_charge : System.Web.UI.Page
{
    ...
    {
        if (txt_grocery_shipping.Text.Length > 0 && txt_restaurant_shipping.Text.Length>0)
        {
            try
            {
               
                int success = bnc.Add_Shipping_Charge(txt_grocery_shipping.Text,txt_restaurant_shipping.Text);
                if (success > 0)
                {
    ...
    protected void btnupdate_ServerClick(object sender, EventArgs e)
    {
        if (txt_grocery_shipping.Text.Length > 0 && txt_restaurant_shipping.Text.Length>0)
        {
            int success = bnc.Edit_Shipping_Charge(txt_grocery_shipping.Text,txt_restaurant_shipping.Text);
